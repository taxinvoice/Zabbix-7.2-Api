...
{
    public UserMacroService(ICore core)
        : base(core, "usermacro")
    {
    }
    ...
    {
        [JsonProperty("globalmacroids")]
        public override IList<string>? Ids { get; set; }
    }

    public string CreateGlobal(GlobalMacro macro)
    {
        string? ids = CreateGlobal(new List<GlobalMacro>() { macro }).FirstOrDefault();
        return Checker.ReturnEmptyStringOrActual(ids);
    }

    public string UpdateGlobal(GlobalMacro macro)
    {
        string? ids = UpdateGlobal(new List<GlobalMacro>() { macro }).FirstOrDefault();
        return Checker.ReturnEmptyStringOrActual(ids);
    }

    public string DeleteGlobal(GlobalMacro macro)
    {
        string? ids = DeleteGlobal(new List<GlobalMacro>() { macro }).FirstOrDefault();
        return Checker.ReturnEmptyStringOrActual(ids);
    }

    public string DeleteGlobal(string id)
    {
        string? ids = DeleteGlobal(new List<string>() { id }).FirstOrDefault();
        return Checker.ReturnEmptyStringOrActual(ids);
    }

    public async Task<string> CreateGlobalAsync(GlobalMacro macro)
    {
        string? ids = (await CreateGlobalAsync(new List<GlobalMacro>() { macro })).FirstOrDefault();
        return Checker.ReturnEmptyStringOrActual(ids);
    }
    ...
    public async Task<string> UpdateGlobalAsync(GlobalMacro macro)
    {
        ...
        string? ids = (await UpdateGlobalAsync(new List<GlobalMacro>() { macro })).FirstOrDefault();
        return Checker.ReturnEmptyStringOrActual(ids);
    }
    ...
    {
        Checker.CheckEntityId(macro);
        string? ids = (await DeleteGlobalAsync(new List<GlobalMacro>() { macro })).FirstOrDefault();
        return Checker.ReturnEmptyStringOrActual(ids);
    }

    public async Task<string> DeleteGlobalAsync(string id)
    {
        string? ids = (await DeleteGlobalAsync(new List<string>() { id })).FirstOrDefault();
        return Checker.ReturnEmptyStringOrActual(ids);
    }
...
{
    [JsonProperty("globalmacro")]
    public bool? GlobalMacro { get; set; }

    [JsonProperty("globalmacroids")]
    public object? GlobalMacroIds { get; set; }
    ...
    [JsonProperty("hostmacroids")]
    public object? HostMacroIds { get; set; }
