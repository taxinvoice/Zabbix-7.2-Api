...
{
    public DeleteService(ICore core, string className)
        : base(core, className)
    {
    }
    ...
    {
        string? ret = Delete(new List<string> { entity.EntityId! }).FirstOrDefault();
        return Checker.ReturnEmptyStringOrActual(ret);
    }

    public virtual string Delete(string id)
    {
        string? ret = Delete(new List<string> { id }).FirstOrDefault();
        return Checker.ReturnEmptyStringOrActual(ret);
    }
    ...
    {
        string? ret = (await DeleteAsync(new List<TEntity> { entity })).FirstOrDefault();
        return Checker.ReturnEmptyStringOrActual(ret);
    }

    public virtual async Task<string> DeleteAsync(string id)
    {
        string? ret = (await DeleteAsync(new List<string> { id })).FirstOrDefault();
        return Checker.ReturnEmptyStringOrActual(ret);
    }
